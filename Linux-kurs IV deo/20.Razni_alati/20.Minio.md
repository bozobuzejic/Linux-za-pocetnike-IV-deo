...
3. Instaliranje i podešavanje MinIO servera sa SSL sertifikatom i ključem (za siguran pristup serveru sa interneta (iz spoljnje mreže) sa HTTPS konekcijom) uz kreiranje vlastitog CA - Certificate Authority (tela za izdavanje SSL sertifikata za lokalni HTTPS)
        ...
        - Pribavljanje SSL ključeva i sertifikata:
        
        openssl genrsa -des3 -out rootCA.key 4096
        openssl req -x509 -new -nodes -key rootCA.key -sha256 -days 1024 -out rootCA.crt
        openssl genrsa -out minio.key 4096
        openssl req -new -sha256 -key minio.key -subj "/C=US/ST=CA/O=MyOrg/OU=ORG_UNIT/CN=bozobuzejic.sytes.net" -reqexts SAN -config <(cat /etc/ssl/openssl.cnf <(printf "\n[SAN]\nsubjectAltName=DNS:bozobuzejic.sytes.net")) -out minio.csr
        
        openssl req -in minio.csr -noout -text                           .....proverimo zahtev za sertifikat
        
        openssl x509 -req -in minio.csr -CA rootCA.crt -CAkey rootCA.key -CAcreateserial -out minio.crt -extfile <(printf "\n[SAN]\nsubjectAltName=DNS:bozobuzejic.sytes.net") -days 500 -sha256 -ext SAN -extensions SAN
        ...
        Mozilla firefox -> Open menu -> Preferences -> Privacy & Security -> Certificates -> View Certificates… -> Authorities -> 
                        -> Import: izaberemo /etc/ssl/certs/rootCA.crt -> Open -> čekiramo: Trust this CA to identify websytes ->
                        -> Ok
                        -> https://bozobuzejic.sytes.net:9001                           .....otvaramo web stranicu sa oznakom da je sertifikat pouzdan
                     
        Chrome          -> Customize and control Google Chrome -> Settings -> Privacy and security -> Security -> Manage cerificates -> 
                        -> Authorities -> Import: izaberemo /etc/ssl/certs/rootCA.crt -> Open -> čekiramo: Trust this CA to identify websytes ->
                        -> Ok
                        -> https://bozobuzejic.sytes.net:9001                           .....otvaramo web stranicu sa oznakom da je sertifikat pouzdan
