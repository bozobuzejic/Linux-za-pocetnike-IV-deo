...
1. Instaliranje i konfiguracija MinIO Object Storage servera (primenjeno na Debian 11 sistemu)
   ...
   1.1. Instaliranje MinIO Object Storage servera
        ...
        sudo useradd -r user-minio -s /sbin/nologin                                         .....kreiramo korisnika za minio
        sudo chown user-minio:user-minio /usr/local/bin/minio                               .....određujemo grupu i korisnika sistemskog minio fajla
   ...
   1.2. Kreiranje Minio skladišta podataka i konfiguracionog fajla
       ...
       -Varijanta 1: Kreiranje particije za Minio Storage
        ...
        sudo chown user-minio:user-minio /mnt/minio                                         .....određujemo grupu i korisnika tačke montiranja particije
       ...
       -Varijanta 2: Kreiranje posebnog foldera za Minio Storage
        ...
        sudo chown user-minio:user-minio /etc/minio                                         .....određujemo grupu i korisnika konfiguracijskih foldera
        sudo chown user-minio:user-minio /usr/local/share/minio
   ...
   1.4. Konfiguracija systemd skripte za pokretanje programa Minio
        ...
        sudo vim minio.service                                                                              .....otvorimo konfiguracioni fajla minio servisa
                 User=user-minio
                 Group=user-minio                                                                                .....izmenimo nazive korisnika i grupu za pristup  
...
2. Konfiguracija MinIO Object Storage servera korišćenjem Minio aplikacije za pristup serveru sa interneta (iz spoljnje mreže)
       ...
       -Pristup spolja, sa interneta:
        ...
        Web browser -> http://bozobuzejic.sytes.net:9001 -> username: usernamkey; password: miniopassword (iz /etc/default/minio fajla) -> 
                    -> kliknemo na Create Bucket dugme da kreiramo bucket (posudu) -> zadamo ime "posudi" -> kliknemo Create Bucket ->
                    -> otpremamo i preuzimamo fajlove iz skladišta (preko minio kontrolne table u web pretraživaču)
...
3. Instaliranje i podešavanje MinIO servera sa SSL sertifikatom i ključem (za siguran pristup serveru sa interneta (iz spoljnje mreže) sa HTTPS konekcijom)    
   uz kreiranje vlastitog CA - Certificate Authority (tela za izdavanje SSL sertifikata za lokalni HTTPS)
        ...
        sed -i 's/User=minio-user/User=user-minio/g' minio.service
        sed -i 's/Group=minio-user/Group=user-minio/g' minio.service                                       .....izmenimo konfiguracioni fajl:
      ...
      *Kreiranje SSL ključeva i Subject Alternative Name (SAN) sertifikata (SAN sertifikat omogućuje da se osigura primarni domen,    
        nakon čega se dodaju dodatni
        ...
        chown bozo-minio:bozo-minio private.key
        chown bozo-minio:bozo-minio public.crt
        ...
        openssl req -new -sha256 -key minio.key -subj "/C=US/ST=CA/O=MyOrg/OU=ORG_UNIT/CN=bozomx.sytes.net" -reqexts SAN -config <(cat /etc/ssl/openssl.cnf <(printf "\n[SAN]\nsubjectAltName=DNS:bozomx.sytes.net")) -out minio.csr
        
        openssl req -in minio.csr -noout -text                           .....proverimo zahtev za sertifikat
        
        openssl x509 -req -in minio.csr -CA rootCA.crt -CAkey rootCA.key -CAcreateserial -out minio.crt -extfile <(printf "\n[SAN]\nsubjectAltName=DNS:bozomx.sytes.net") -days 500 -sha256 -ext SAN -extensions SAN
        ...
        Web browser -> https://bozomx.sytes.net:9000                            .....otvaramo web stranicu sa oznakom da je veza bezbedna (sertifikat pouzdan)
        ...
        Napomena: Generisali smo sopstveni root sertifikat i privatni ključ. Zatim dodamo autorizovan root sertifikat na one uređaje na kojima nam to odgovara. 
      ...
      *Pribavljanje autorizovanih SSL ključeva sa NoIp portala
        Generisanje zahteva za potpisivanje sertifikata (CSR) i privatnog ključa za server uz pomoć NoIp DNS 
        menadžera (na kome je kreiran hostname bozomx.sytes.net) kako bi se dobio besplatan TrustCor Standard DV sertifikat 
        ...
        sudo openssl x509 -outform pem -in server.pem-chain -out public.crt                   .....kreiramo sertifikat konvertovanjem .pem-chain fajla
     
        chown bozo-minio:bozo-minio private.key
        chown bozo-minio:bozo-minio public.crt                                                .....dodelimo vlasnička prava korisniku bozo-minio nad fajlovima
        
  
        -Pristup spolja, sa interneta (korišćenjem autorizovanog sertifikata od strane NoIp menadžera):
        
      
        Web browser -> https://bozomx.sytes.net:9000 -> username: usernamkey; password: miniopassword -> Login    ....."veza je bezbedna (sertifikat je pouzdan)"  
...
4. Upotreba mc (MiniIO klijent) za upravljanje minio servisom
        ...
        mc config host add minio https://bozobuzejic.sytes.net:9001 usernamkey miniopassword .....sa mc komandom   
